...
def count_convNd(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    ...
    m.out_shape = torch.Tensor([int(i) for i in y.shape[1:]])
...
def count_linear(m, x, y):
    ...
    ## add output size and shape
    m.total_ops += torch.Tensor([int(total_ops)])
    m.out_size = torch.Tensor([int(y.numel())])
    m.out_shape = torch.Tensor([int(i) for i in y.shape[1:]])

    ## for csv export
    m.kernel_size = [int(x[0].numel()/m.in_features)]
    m.in_channels = m.in_features
    m.groups = 1
    m.out_channels = y.shape[-1]
###
...
def profile_prune(model, inputs, macs=False, custom_ops=None, verbose=False, \
    prune=True, mode=0, file=None, show_layer=True, inMega=False, export_csv=False, threed=False):
    '''
    ...
    if export_csv:
        with open('profile.csv', 'w', newline='') as f:
            ...
            k_list = ['Kernel_H', 'Kernel_W']
            out_list = ['Out_H', 'Out_W']
            if threed:
                k_list = ['Kernel_D'] + k_str
                out_list = ['Out_D'] + out_str
            csvwriter.writerow(['Layer'] + k_list + ['Chan_In', 'Groups', 'Chan_Out'] \
                + out_list + ['Params (M)', 'Operations (GOPs)'])
    ...
    def add_hooks(m):
        ...
        m.register_buffer('out_shape', torch.zeros(1))
    ...
    for name, m in model.named_modules():  # layer name (...conv, fc, etc.)
        ...
        fshape[name] = [int(i) for i in m.out_shape]
        if params[name] == 0:
            continue
        ...
        if not prune and show_layer and (isinstance(m, nn.modules.conv._ConvNd) or isinstance(m, nn.Linear)):  # conv or fc
            ...
            print('   params: {:.2f}M   ofm shape: {:<12s}   {}: {:.2f}{}'.format(
                params[name]/1e6, str(fshape[name]), op_name, ops[name]/op_num, op_unit))
            print('')

            if export_csv:
                with open('profile.csv', 'a', newline='') as f:
                    csvwriter = csv.writer(f)
                    kernel_list = m.kernel_size
                    fshape_list = fshape[name]
                    if isinstance(m, nn.Linear):
                        kernel_list += [1]
                        fshape_list = fshape_list[::-1]
                        len_fshape = len(fshape[name]) - 1  # first is output channel
                        assert len_fshape <= 2
                        fshape_list += [1] * (2 - len_fshape)
                        if threed:
                            kernel_list += [1]
                            fshape_list += [1]
                    csvwriter.writerow([name, *m.kernel_size, m.in_channels//m.groups, m.groups, *fshape_list, \
                        '{:.4f}'.format(params[name]/1e6), '{:.4f}'.format(ops[name]/1e9)])
    ...
    for n, m in model.named_modules():
        ...
        if "out_shape" in m._buffers:
            m._buffers.pop("out_shape")
    ...
    for name, m in model.named_modules():  # layer name (...conv, fc, etc.)
        ...
        if show_layer and (isinstance(m, nn.modules.conv._ConvNd) or isinstance(m, nn.Linear)):  # conv or fc
            ...
            print('   params rmn/tot: {:.2f}M/{:.2f}M ({:.2f}x, {:.2f} pruned)   ofm shape: {:<12s}   '
                '{} rmn/tot: {:.2f}{}/{:.2f}{}'.format(
                params_rmn[name]/1e6, params[name]/1e6, params[name]/params_rmn[name], 1-params_rmn[name]/params[name], \
                str(fshape[name]), op_name, ops_rmn[name]/op_num, op_unit, ops[name]/op_num, op_unit))
            print('')
            print('')
